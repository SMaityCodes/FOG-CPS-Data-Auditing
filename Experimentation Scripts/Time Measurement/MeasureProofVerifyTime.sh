...
executionCommand="./DataAudit VerifyProof Params.bin POP.bin metaData.bin ChalStr.txt"
TESTCOUNT=70
TotTime=0
counter=0
...
do
	...
	x=$(./DataAudit ProofGen Params.bin $dataFileName ChalStr.txt) # taking the ouput in a varialbe so that it is not printed on screen
